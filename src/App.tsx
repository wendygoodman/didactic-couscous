...
const PRODUCTS: Product[] = [
  {"name":"Canva Pro","category":["Design"],"prices":{"sixMonth":null,"oneYear":null,"lifetime":10},"devices":Infinity as any,"description":"Upgrade on your own email. Premium templates, stock media & HQ download.","link":"#"},
  {"name":"Canva Pro + AI","category":["Design","AI"],"prices":{"sixMonth":null,"oneYear":null,"lifetime":30},"devices":Infinity as any,"description":"All Pro features + Magic Studio AI, background remover, custom fonts.","link":"#"},
  ...
  {"name":"Spotify Premium","category":["Streaming","Music"],"prices":{"sixMonth":null,"oneYear":30,"lifetime":40},"devices":Infinity as any,"description":"Upgrade on your email. Ad-free, offline, HQ audio.","link":"#"},
  ...
  {"name":"Duolingo Plus","category":["Education"],"prices":{"sixMonth":8,"oneYear":15,"lifetime":20},"devices":Infinity as any,"description":"Upgrade on your email. Ad-free, offline lessons, progress sync.","link":"#"},
  ...
  {"name":"Gemini Pro + Veo 3","category":["AI","Video"],"prices":{"sixMonth":null,"oneYear":45,"lifetime":null},"devices":Infinity as any,"description":"New private account. Multimodal AI + Veo 3 cinema video AI.","link":"#"},
  ...
  {"name":"YouTube Premium","category":["Streaming","Music"],"prices":{"sixMonth":null,"oneYear":40,"lifetime":80},"devices":Infinity as any,"description":"Upgrade on your email. Ad-free YouTube, downloads, background play.","link":"#"},
  ...
  {"name":"Google Drive (Upgrade)","category":["Cloud","Productivity"],"prices":{"sixMonth":null,"oneYear":35,"lifetime":null},"devices":Infinity as any,"description":"Upgrade on your email. Extra storage & Workspace perks.","link":"#"}
]
...
const TITLE_COLORS: Record<string,string> = {
  'Canva Pro':'#a78bfa','Canva Pro + AI':'#a78bfa','CapCut':'#e5e7eb','Grammarly':'#00A82D','Spotify':'#1DB954','QuillBot':'#2EAC68',
  'Duolingo':'#78C800','ChatGPT':'#10a37f','Gemini':'#22d3ee','Prime Video':'#00A8E1','Netflix':'#E50914','YouTube':'#FF0000',
  'NordVPN':'#2F6CE5','IPVanish':'#8BC34A','Surfshark':'#00C3C1','Coursera':'#2A73CC','MS Office':'#D83B01','Windows':'#00A4EF',
...
const titleColor = (name:string) => {
  ...
  return matched ? matched[1] : '#d1d5db'
}
const adjustOneCent = (v:number) => Math.max(0, Math.round(v*100)-1)/100

function Price({ value }:{ value:number|null }){
  if (value == null) return <span className="text-zinc-500">—</span>
  return <span className="font-bold drop-shadow" style={{ color: MATCHA }}>{currencyFormatter.format(adjustOneCent(value))}</span>
}
...
export default function App(){
  ...
  const filtered = useMemo(()=>{
    ...
    const getVal = (p:Product) => {
      switch (sortKey){
        ...
        case 'sixMonth': return p.prices.sixMonth ?? Number.POSITIVE_INFINITY
        case 'oneYear': return p.prices.oneYear ?? Number.POSITIVE_INFINITY
        case 'lifetime': return p.prices.lifetime ?? Number.POSITIVE_INFINITY
        case 'devices': return (p.devices as any) === Infinity ? 9999 : p.devices
      }
    }
    list.sort((a,b)=>{
      const va:any = getVal(a), vb:any = getVal(b)
  ...
  const exportCSV = () => {
    ...
    a.href = url; a.download = `pricing-catalog-${Date.now()}.csv`; a.click()
      ...
      <div className="mx-auto max-w-7xl px-4 py-10">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={()=> setView(view==='grid' ? 'table' : 'grid')} className="rounded-2xl border border-zinc-700 bg-zinc-900/50 px-3 py-2 hover:bg-zinc-900 transition">
              {view === 'grid' ? <Table className="h-4 w-4" /> : <LayoutGrid className="h-4 w-4" />}
            </button>
            <button onClick={()=> location.reload()} className="rounded-2xl border border-zinc-700 bg-zinc-900/50 px-3 py-2 hover:bg-zinc-900 transition">
              <span className="inline-flex items-center gap-2"><RefreshCcw className="h-4 w-4" /> Refresh</span>
            </button>
            <button onClick={exportCSV} className="rounded-2xl border border-zinc-700 bg-zinc-900/50 px-3 py-2 hover:bg-zinc-900 transition">
            ...
            <AnimatePresence>
              {filtered.map(p => (
                <motion.div key={p.name} initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} exit={{opacity:0, y:10}}>
                  <div className="group rounded-2xl border border-zinc-800 bg-gradient-to-b from-zinc-900/70 to-black/60 backdrop-blur-sm hover:shadow-2xl hover:shadow-zinc-900/30 transition-all p-4">
